// Package aider implements the ExecutionEngine interface for the Aider
// CLI, translating tasks into execution specs that run Aider inside
// Kubernetes Jobs.
//
// Aider does not support a hooks system, so guard rails are enforced via
// prompt-based instructions (see oss-plan section 6.3). Repository
// conventions are sourced from .aider/conventions.md rather than CLAUDE.md.
package aider
...
const (
	...
	defaultImage = "ghcr.io/robodev-inc/engine-aider:latest"
...
// ModelProvider indicates which LLM provider Aider should use.
type ModelProvider string

const (
	// ProviderAnthropic configures Aider to use the Anthropic API.
	ProviderAnthropic ModelProvider = "anthropic"

	// ProviderOpenAI configures Aider to use the OpenAI API.
	ProviderOpenAI ModelProvider = "openai"
)

// AiderEngine implements engine.ExecutionEngine for the Aider CLI.
type AiderEngine struct {
	// provider determines which LLM provider Aider will use.
	// Defaults to ProviderAnthropic if not set.
	provider ModelProvider
}

// Option is a functional option for configuring the AiderEngine.
type Option func(*AiderEngine)

// WithProvider sets the model provider for the Aider engine.
func WithProvider(p ModelProvider) Option {
	return func(e *AiderEngine) {
		e.provider = p
	}
}

// New returns a new AiderEngine with the given options applied.
func New(opts ...Option) *AiderEngine {
	e := &AiderEngine{
...
func (e *AiderEngine) Name() string {
	return engineName
}

// InterfaceVersion returns the version of the ExecutionEngine interface
// this engine implements.
func (e *AiderEngine) InterfaceVersion() int {
	return interfaceVersion
}

// BuildExecutionSpec translates a task and engine configuration into an
// engine-agnostic ExecutionSpec for running Aider. Because Aider lacks a
// hooks system, guard rails are embedded directly in the prompt text.
func (e *AiderEngine) BuildExecutionSpec(task engine.Task, config engine.EngineConfig) (*engine.ExecutionSpec, error) {
	if task.ID == "" {
...
func (e *AiderEngine) buildSecretEnv() map[string]string {
	switch e.provider {
	case ProviderOpenAI:
...
// BuildPrompt constructs the task prompt for Aider from the task's title,
// description, and any additional metadata. Guard rails are appended
// directly to the prompt text because Aider does not support a hooks
// system for enforcing them at the tool level.
func (e *AiderEngine) BuildPrompt(task engine.Task) (string, error) {
	if task.Title == "" {
	...
	// Aider uses .aider/conventions.md for repository context.
	b.WriteString("## Repository Context\n\n")
	b.WriteString("Refer to the .aider/conventions.md file in the repository for coding\n")
	b.WriteString("conventions, project structure, and contribution guidelines.\n")
	b.WriteString("If an .aider.conf.yml file is present, it will be used for Aider configuration.\n\n")
