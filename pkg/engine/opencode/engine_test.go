...
var _ engine.ExecutionEngine = (*OpenCodeEngine)(nil)
...
func TestBuildExecutionSpec(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			task: baseTask,
			config: engine.EngineConfig{
				Image:          "my-registry/opencode:v2",
				TimeoutSeconds: 3600,
			},
			check: func(t *testing.T, spec *engine.ExecutionSpec) {
				assert.Equal(t, "my-registry/opencode:v2", spec.Image)
			},
		},
		{
			...
			check: func(t *testing.T, spec *engine.ExecutionSpec) {
				...
				assert.Equal(t, "opencode", spec.Command[0])
				assert.Equal(t, "--non-interactive", spec.Command[1])
				assert.Equal(t, "--message", spec.Command[2])
		...
		},
		{
			...
			check: func(t *testing.T, spec *engine.ExecutionSpec) {
				...
				_, hasOpenAI := spec.SecretEnv["OPENAI_API_KEY"]
				assert.False(t, hasOpenAI, "should not have OpenAI key with Google provider")
			},
		},
		{
