...
func TestShortcutBackend_PollReadyTickets(t *testing.T) {
	stories := []scStory{
		{
			ID:          42,
			Name:        "Fix login bug",
			...
			Labels:      []scLabel{{Name: "robodev"}, {Name: "bug"}},
		},
		{
			ID:          99,
			Name:        "Refactor auth",
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "robodev", sr.LabelName)
	...
	assert.Equal(t, "story", tickets[0].TicketType)
	assert.Equal(t, []string{"robodev", "bug"}, tickets[0].Labels)
	assert.Equal(t, "https://app.shortcut.com/org/story/42", tickets[0].ExternalURL)
...
func TestShortcutBackend_PollReadyTickets_ExcludeLabels(t *testing.T) {
	stories := []scStory{
		...
		},
		{
			...
			Labels: []scLabel{{Name: "robodev"}, {Name: "in-progress"}},
		},
		{
			ID:     3,
			Name:   "Failed story",
			AppURL: "https://app.shortcut.com/org/story/3",
			Labels: []scLabel{{Name: "robodev"}, {Name: "robodev-failed"}},
		},
		{
			ID:     4,
			Name:   "Clean story",
