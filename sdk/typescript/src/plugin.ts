/**
 * Base class helpers for authoring RoboDev gRPC plugins in TypeScript.
 *
 * Every RoboDev plugin is an out-of-process gRPC server. The controller
...
/** Default port used when ROBODEV_PLUGIN_PORT is not set (0 = ephemeral). */
const DEFAULT_PORT = parseInt(process.env["ROBODEV_PLUGIN_PORT"] ?? "0", 10);

/**
 * PluginBase is the abstract base class for all RoboDev TypeScript plugins.
 *
...
export abstract class PluginBase {
   ...
   * {@link registerRoutes}.
   *
   * The listening port is taken from the `ROBODEV_PLUGIN_PORT` environment
   * variable. When not set the server binds to an ephemeral port on
   * `127.0.0.1`.
   *
   * @param port - Optional TCP port override. Defaults to `ROBODEV_PLUGIN_PORT`
   *               or 0 (ephemeral).
   */
  serve(port: number = DEFAULT_PORT): void {
    ...
    server.listen(port, "127.0.0.1", () => {
      ...
      console.log(`RoboDev plugin listening on 127.0.0.1:${actualPort}`);
    });
  }
}
